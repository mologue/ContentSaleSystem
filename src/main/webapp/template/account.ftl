...
<div class="g-doc">
    ...
    <#if buyList?? || !buyList?has_content>
        <table class="m-table m-table-row n-table g-b3">
            ...
            <tbody>
                <#list buyList as x>
                    <#assign total = total + x.amount*x.price>
                <tr>
                    <td><a href="/show?id=${x.orderId}"><img src="${x.picture}" alt=""></a></td>
                    <td><h4><a href="/show?id=${x.orderId}">${x.title}</a></h4></td>
                    <#--<td><span class="v-time">${x.date?number_to_datetime?string("yyyy-MM-dd HH:mm")}</span></td>-->
                    <td><span class="v-time">${x.date}</span> </td>
                    <td><span class="v-num">${x.amount}</span></td>
